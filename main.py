...
def load_random_sound():
    return os.path.join(
        "assets", "sounds", str(random.randrange(1, SOUNDS_COUNT + 1)) + ".mp3"
    )
...
def get_fruits(db_path):
    ...
    result = cur.execute(
        "SELECT filename_full, filename_half FROM fruits_table"
    ).fetchall()
    return result
...
class Fruit(pygame.sprite.Sprite):
    ...
    def __init__(self, *group, filename_full, filename_half):
        ...
        self.image_half = pygame.transform.scale(
            self.image_half, (FRUIT_SIDE, FRUIT_SIDE)
        )
        ...
        if self.rect.collidepoint(pygame.mouse.get_pos()):
            ...
            if self.rect.x - self.hit_x_pos < 150:
                self.create_particles(
                    (self.rect.x + (FRUIT_SIDE // 2), self.rect.y + (FRUIT_SIDE // 2))
                )
        return False
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == FRUITEVENT:
                ...
                Fruit(
                    all_sprites,
                    filename_full=fruit_tuple[0],
                    filename_half=fruit_tuple[1],
                )
